...
class GlossaryLogger(logging.Logger):
    ...
    def __init__(self, logger_type: LoggerType, name: str = __name__, level: int = logging.DEBUG):
        ...
        # Формируем единый формат логирования
    ...
    def _determine_logpath(self) -> str:
        ...
        if self.logger_type == LoggerType.SCHEDULER:
            if env == "docker":
                return settings.scheduler.logs_contr_path
            else:
                return settings.scheduler.logs_host_path
        if self.logger_type == LoggerType.ARQ:
            if env == "docker":
                return settings.arq.logs_contr_path
            else:
                return settings.arq.logs_host_path
